    ...
    internal static class OrderByMethodToPipelineTranslator
    {
        ...
        private static AstPipeline CombineSortStages(AstPipeline pipeline, AstStage[] newSortStages)
        {
            ...
            static AstPipeline Combine1And3(AstPipeline pipeline, AstStage[] oldSortStages, AstStage[] newSortStages)
            {
                ...
                // { $sort : { f1 : d1, ..., fj : dj } }
                // new:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : expr1, _id : 0 } }
                // { $sort : { _key1 : dj+1 } }
                // { $replaceRoot : { newRoot : "$_sortedDocument" } }
                // combined:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : expr1, _id : 0 } }
                // { $sort : { '_sortedDocument.f1' : d1, ..., '_sortedDocument.fj' : dj, _key1 : dj+1 } }
                // { $replaceRoot : { newRoot : "$_sortedDocument" } }
                ...
                var combinedSortStage = AstStage.Sort(
                    oldSortStage.Fields.Select(f => AstSort.Field("_sortedDocument." + f.Path, f.Order))
            ...
            {
                // old:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : expr1, ..., _keyj : exprj, _id : 0 } }
                // { $sort : { f1 : d1, ..., fk : dk } }
                // { $replaceRoot : { newRoot : "$_sortedDocument" } }
                // new:
                // { $sort : { fk+1 : dk+1 } }
                // combined:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : expr1, ..., _keyj : exprj, _id : 0 } }
                // { $sort : { f1 : d1, ..., fk : dk, '_sortedDocument.fk+1' : dk+1 } }
                // { $replaceRoot : { newRoot : "$_sortedDocument" } }
                ...
                var combinedSortStage = AstStage.Sort(
                    oldSortStage.Fields
                    .Append(newSortStage.Fields.Select(f => AstSort.Field("_sortedDocument." + f.Path, f.Order)).Single()));
            ...
            {
                // old:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : oldExpr1, ..., _keyj : oldExprj, _id : 0 } }
                // { $sort : { f1 : d1, ..., fk : dk } }
                // { $replaceRoot : { newRoot : "$_sortedDocument" } }
                // new:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : newExpr, _id : 0 } }
                // { $sort : { _key1 : dnew } }
                // { $replaceRoot : { newRoot : "$_sortedDocument" } }
                // combined:
                // { $project : { _sortedDocument : "$$ROOT", _key1 : oldExpr1, ..., _keyj : oldExprj, _keyj+1 : newExpr, _id : 0 } }
                ...
                var newKey = $"_key{j + 1}";
                var newExpr = ((AstProjectStageSetFieldSpecification)newProjectStage.Specifications[1]).Value;
                var dNew = newSortStage.Fields.Single().Order;

                var combinedProjectStage = AstStage.Project(
                    oldProjectStage.Specifications.Take(oldProjectStage.Specifications.Count - 1)
                    .Append(AstProject.Set(newKey, newExpr))
                    .Append(AstProject.Exclude("_id")));
                var combinedSortStage = AstStage.Sort(
                    oldSortStage.Fields
        ...
        private static AstStage[] CreateSortStages(string methodName, AggregationExpression keySelectorTranslation)
        {
            ...
            else
            {
                var projectStage = AstStage.Project(
                    AstProject.Set("_sortedDocument", AstExpression.FieldPath("$$ROOT")),
                ...
                var replaceRootStage = AstStage.ReplaceRoot(AstExpression.FieldPath("$_sortedDocument"));
