...
namespace MongoDB.Driver.Linq.Linq3Implementation.Translators.ExpressionToAggregationExpressionTranslators.MethodTranslators
{
    internal static class CreateMethodToAggregationExpressionTranslator
    {
        ...
        public static AggregationExpression Translate(TranslationContext context, MethodCallExpression expression)
        {
            ...
            {
                var items = new List<AstExpression>(arguments.Count);
                ...
                for (var i = 0; i < arguments.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        itemSerializer = CreateTupleSerializer(method, new[] { valueTranslation.Serializer });
                    }
                    items.Add(item);
                    itemSerializers.Add(itemSerializer);
                }

                var ast = AstExpression.ComputedArray(items);
                var tupleSerializer = CreateTupleSerializer(method, itemSerializers);
        ...
        private static IBsonSerializer CreateTupleSerializer(MethodInfo method, IEnumerable<IBsonSerializer> itemSerializers)
        {
            return method.IsOneOf(__tupleCreateMethods) ?
