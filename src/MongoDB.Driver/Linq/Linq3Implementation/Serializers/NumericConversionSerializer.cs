...
{
    /// <summary>
    /// Static factory class for NumericConversionSerializer.
    /// </summary>
    internal static class NumericConversionSerializer
    {
        ...
        {
            var numericConversionSerializerType = typeof(NumericConversionSerializer<,>).MakeGenericType(sourceType, targetType);
    ...
    internal class NumericConversionSerializer<TSource, TTarget> : SerializerBase<TTarget>, IBsonDocumentSerializer
    {
        ...
        {
            if (_sourceSerializer is IBsonDocumentSerializer documentSerializer)
            {
                return documentSerializer.TryGetMemberSerializationInfo(memberName, out serializationInfo);
            }

            throw new InvalidOperationException($"The class {_sourceSerializer.GetType().FullName} does not implement IBsonDocumentSerializer.");
        }

        private object Convert(TypeCode fromTypeCode, TypeCode toTypeCode, object value)
        {
        ...
        private TTo ConvertChecked<TFrom, TTo>(TFrom value)
        {
            ...
            var converted = (TTo)Convert(fromTypeCode, toTypeCode, value);
            var convertedBack = Convert(toTypeCode, fromTypeCode, converted);
