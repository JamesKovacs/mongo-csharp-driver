...
namespace MongoDB.Driver.Linq.Linq3Implementation.Serializers
{
    ...
    {
        public static IBsonSerializer Create(IBsonSerializer serializer)
        {
            var valueType = serializer.ValueType;
            if (valueType.IsNullable())
            {
                var nullableSerializer = (INullableSerializer)serializer;
                var enumSerializer = nullableSerializer.ValueSerializer ;
                var enumUnderlyingTypeSerializer = Create(enumSerializer);
                return NullableSerializer.Create(enumUnderlyingTypeSerializer);
            }
            else
            {
                var enumSerializer = serializer;
