...
if [ "$DOTNET_VERSION" != "$DOTNET_INSTALLED_VERSION" ]; then
    ...
    bash "$SCRIPT_DIR/.dotnet/dotnet-install.sh" --version 1.1.14 --install-dir .dotnet --no-path
    bash "$SCRIPT_DIR/.dotnet/dotnet-install.sh" --version 2.1.810 --install-dir .dotnet --no-path
