...
namespace MongoDB.Driver.Tests.Communication.Security
{
    ...
    public class OidcAuthenticationProseTests : LoggableTestClass
    {
        ...
        [Theory]
        [InlineData("test_user1", "mongodb://localhost:27017/?authMechanism=MONGODB-OIDC&authMechanismProperties=PROVIDER_NAME:aws")]
        public async Task ShouldDoSomething(string user, string connectionString)
        {
            var settings = GetOidcEffectiveMongoClientSettings(MongoClientSettings.FromConnectionString(connectionString));
            var client1 = DriverTestConfiguration.CreateDisposableClient(settings);

            var db = client1.GetDatabase(DatabaseName);
            ...
            var client2 = DriverTestConfiguration.CreateDisposableClient(settings);
            var collection2 = db.GetCollection<BsonDocument>("collName");

            var task1 = collection.FindAsync(Builders<BsonDocument>.Filter.Empty);
            var task2 = collection2.FindAsync(Builders<BsonDocument>.Filter.Empty);
            var data = (await task1).ToList();
            var data2 = (await task2).ToList();

            data.First().Should().Match(d => d["abc"] == 10);
        }
